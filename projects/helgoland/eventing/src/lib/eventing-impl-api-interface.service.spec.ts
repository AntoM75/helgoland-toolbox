...
describe('EventingImplApiInterface', () => {
  ...
  it('get publications with filter', inject([EventingImplApiInterface], (service: EventingImplApiInterface) => {
    ...
    service.getPublications(testUrl, filterParameter).subscribe(data => expect(data).toEqual(testData));
    const req = httpTestingController.expectOne((request) => request.method === 'GET');
    expect(req.request.params.get('expanded')).toBeTruthy(filterParameter.expanded);
    expect(req.request.params.get('offset') + '').toEqual(filterParameter.offset + '');
    expect(req.request.params.get('limit') + '').toEqual(filterParameter.limit + '');
    expect(req.request.params.get('feature') + '').toEqual(filterParameter.feature + '');
    req.flush(testData);
  }));

  it('get publications with basic auth', inject([EventingImplApiInterface], (service: EventingImplApiInterface) => {
    const testData: PublicationResults = require('../test-data/publications.json');
    const options: HttpRequestOptions = createRequestOptions();
  ...
  it('get publication', inject([EventingImplApiInterface], (service: EventingImplApiInterface) => {
    const testData: Event = require('../test-data/publication.json');
    service.getPublication(testData.id, testUrl).subscribe(data => expect(data.id).toEqual(testData.id));
    const req = httpTestingController.expectOne(testUrl + 'publications/' + testData.id);
    expect(req.request.method).toEqual('GET');
    req.flush(testData);
  }));

  it('get publication with basic auth', inject([EventingImplApiInterface], (service: EventingImplApiInterface) => {
    const testData: Event = require('../test-data/publication.json');
    const options = createRequestOptions();
  ...
  it('get notifications with filter', inject([EventingImplApiInterface], (service: EventingImplApiInterface) => {
    ...
    service.getNotifications(testUrl, filterParameter).subscribe(data => expect(data).toEqual(testData));
    const req = httpTestingController.expectOne((request) => request.method === 'GET');
    expect(req.request.params.get('expanded')).toBeTruthy(filterParameter.expanded);
    expect(req.request.params.get('offset') + '').toEqual(filterParameter.offset + '');
    expect(req.request.params.get('limit') + '').toEqual(filterParameter.limit + '');
    expect(req.request.params.get('publications') + '').toEqual(filterParameter.publications + '');
    req.flush(testData);
  }));

  it('get notifications with basic auth', inject([EventingImplApiInterface], (service: EventingImplApiInterface) => {
    const testData: NotificationResults = require('../test-data/notifications.json');
    const options: HttpRequestOptions = createRequestOptions();
  ...
  it('get notification', inject([EventingImplApiInterface], (service: EventingImplApiInterface) => {
    const testData: Event = require('../test-data/notification.json');
    service.getNotification(testData.id, testUrl).subscribe(data => expect(data.id).toEqual(testData.id));
    const req = httpTestingController.expectOne(testUrl + 'notifications/' + testData.id);
    expect(req.request.method).toEqual('GET');
    req.flush(testData);
  }));

  it('get notification with basic auth', inject([EventingImplApiInterface], (service: EventingImplApiInterface) => {
    const testData: Event = require('../test-data/notification.json');
    const options = createRequestOptions();
